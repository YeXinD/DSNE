...
def bottleneck(inputs,
    ...
    with variable_scope.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        ...
        random_range = 0.12
        weight1 = tf.random_uniform((), minval=1 - random_range, maxval=1 + random_range)
        # weight2 = tf.random_uniform((), minval=1.0 - random_range, maxval=1.0 + random_range)
        output = weight1 * (1 * shortcut + 0.8 * residual)


        # output = weight * (shortcut + residual)


        # weight = tf.random_normal((), mean=1.0, stddev=0.04,)
        # weight = tf.random_uniform((depth,), minval=1 - random_range, maxval=1 + random_range)  # weight维度为1就行，比维度(depth,)好
        # output = weight * (shortcut + residual)

        # weight1 = tf.random_uniform((), minval=1 - random_range, maxval=1 + random_range)
        # weight2 = tf.random_uniform((), minval=1 - random_range, maxval=1 + random_range)
        # output = weight1 * shortcut + weight2 * residual  # 不比上面weight * (shortcut + residual)好
